...
annot <- annot %>% filter(Annotation1 == "TME")
annot$cNMF_GEP <- gsub("GEP_","mGEP",annot$cNMF_GEP)
TME_GEP <- paste("mGEP", sort(as.numeric(gsub("mGEP","",unique(annot$cNMF_GEP)))),sep="")
...
for(thresh in c(0.01)){ 
    for(s in 1:length(GEPs)){
    
        df1 <- data.frame(matrix(0, nrow = 6, ncol = 4, dimnames = list(c("Brain","Tumor","Tumor_D1","Tumor_D2","Tumor_D3","Tumor_D4"), c("nr_GEP","nr_nonGEP", "pct_GEP","pct_nonGEP"))))
        MD_subset <- MD #%>% filter(Sample_RepMerged_SideSep == GEPs[s])
    
        mCon_sub <- mCon[MD_subset$Barcode,]
        GEP_spots <- rownames(mCon_sub)[which(mCon_sub[,GEPs[s]] > thresh)]
        
        #Brain
        GEP_spots <- unique(GEP_spots)
        nonGEP_spots <- setdiff(MD_subset$Barcode, GEP_spots)
        df1$nr_GEP[1] <- length(GEP_spots)
        df1$nr_nonGEP[1] <- length(nonGEP_spots)
        df1$pct_GEP[1] <- length(GEP_spots)/length(MD_subset$Barcode)
        df1$pct_nonGEP[1] <- length(nonGEP_spots)/length(MD_subset$Barcode)

        #Tumor
        Tumor_spots <- MD_subset$Barcode[which(MD_subset$Hs_admix > 0.05)]
        Tumor_spots_GEP <- intersect(Tumor_spots, GEP_spots)
        Tumor_spots_nonGEP <- setdiff(Tumor_spots, Tumor_spots_GEP)
        df1$nr_GEP[2] <- length(Tumor_spots_GEP)
        df1$nr_nonGEP[2] <- length(Tumor_spots_nonGEP)
        df1$pct_GEP[2] <- length(Tumor_spots_GEP)/length(Tumor_spots)
        df1$pct_nonGEP[2] <- length(Tumor_spots_nonGEP)/length(Tumor_spots)

        #Tumor_D1
        TumorD1_spots <- MD_subset$Barcode[which(MD_subset$Density_group == "D1")]
        TumorD1_spots_GEP <- intersect(TumorD1_spots, GEP_spots)
        TumorD1_spots_nonGEP <- setdiff(TumorD1_spots, TumorD1_spots_GEP)
        df1$nr_GEP[3] <- length(TumorD1_spots_GEP)
        df1$nr_nonGEP[3] <- length(TumorD1_spots_nonGEP)
        df1$pct_GEP[3] <- length(TumorD1_spots_GEP)/length(TumorD1_spots)
        df1$pct_nonGEP[3] <- length(TumorD1_spots_nonGEP)/length(TumorD1_spots)

        #Tumor_D2
        TumorD2_spots <- MD_subset$Barcode[which(MD_subset$Density_group == "D2")]
        TumorD2_spots_GEP <- intersect(TumorD2_spots, GEP_spots)
        TumorD2_spots_nonGEP <- setdiff(TumorD2_spots, TumorD2_spots_GEP)    
        df1$nr_GEP[4] <- length(TumorD2_spots_GEP)
        df1$nr_nonGEP[4] <- length(TumorD2_spots_nonGEP)
        df1$pct_GEP[4] <- length(TumorD2_spots_GEP)/length(TumorD2_spots)
        df1$pct_nonGEP[4] <- length(TumorD2_spots_nonGEP)/length(TumorD2_spots)

        #Tumor_D3
        TumorD3_spots <- MD_subset$Barcode[which(MD_subset$Density_group == "D3")]
        TumorD3_spots_GEP <- intersect(TumorD3_spots, GEP_spots)
        TumorD3_spots_nonGEP <- setdiff(TumorD3_spots, TumorD3_spots_GEP) 
        df1$nr_GEP[5] <- length(TumorD3_spots_GEP)
        df1$nr_nonGEP[5] <- length(TumorD3_spots_nonGEP)
        df1$pct_GEP[5] <- length(TumorD3_spots_GEP)/length(TumorD3_spots)
        df1$pct_nonGEP[5] <- length(TumorD3_spots_nonGEP)/length(TumorD3_spots)

        #Tumor_D4
        TumorD4_spots <- MD_subset$Barcode[which(MD_subset$Density_group == "D4")]
        TumorD4_spots_GEP <- intersect(TumorD4_spots, GEP_spots)
        TumorD4_spots_nonGEP <- setdiff(TumorD4_spots, TumorD4_spots_GEP) 
        df1$nr_GEP[6] <- length(TumorD4_spots_GEP)
        df1$nr_nonGEP[6] <- length(TumorD4_spots_nonGEP)
        df1$pct_GEP[6] <- length(TumorD4_spots_GEP)/length(TumorD4_spots)
        df1$pct_nonGEP[6] <- length(TumorD4_spots_nonGEP)/length(TumorD4_spots)

        cont_tab[[s]] <- df1
    }
    #saveRDS(cont_tab, paste("v2_ALL_mGEPs_nrSpots_per_Density_usagethreshold", thresh, "_nrSpotThresh", NRSPOT, ".rds",sep=""))


    pdf(paste("ALL_mGEPs_chisq_usagethreshold", thresh, "_nrSpotThresh", NRSPOT, ".pdf",sep=""), width = 15, height = 5)
    for(s in 1:length(cont_tab)){
        ...
        if(length(which(rowSums(df1) == 0)) > 0){
            chsq_DFlist[[1]] <- df1[which(rownames(df1) %in% intersect(rownames(df1), c("Brain","Tumor","Tumor_D1","Tumor_D2","Tumor_D3","Tumor_D4"))),]
            chsq_DFlist[[1]] <- chsq_DFlist[[1]][-which(rowSums(chsq_DFlist[[1]]) == 0),]
        }else{
            chsq_DFlist[[1]] <- df1[which(rownames(df1) %in% intersect(rownames(df1), c("Brain","Tumor","Tumor_D1","Tumor_D2","Tumor_D3","Tumor_D4"))),]
        }
        ...
        TAB_sub1 <- data.frame(GEP = names(cont_tab)[s],
...
write.table(TAB_all, "mGEP_TME_chisq_all_samples_per_program.txt", sep="\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
