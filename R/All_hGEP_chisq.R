...
for(NRSPOT in 50){
    for(thresh in 0.01){
        ...
        for(s in 1:length(GEPs)){
            ...
            GEP_spots <- names(which(as.matrix(hCon)[,GEPs[s]] > thresh)) ## USAGE THRESHOLD
            barcode_sub <- MD %>% filter(Barcode %in% GEP_spots)
            
            sample_freq <- data.frame(table(barcode_sub$Sample_RepMerged_SideSep))
            sample_freq_sub <- sample_freq %>% filter(Freq > NRSPOT) ## NR SPOT THRESHOLD
            sample_sub <- barcode_sub %>% filter(Sample_RepMerged_SideSep %in% as.character(sample_freq_sub$Var1))
            GEP_spots <- sample_sub$Barcode

            MD_subset <- MD %>% filter(Sample_RepMerged_SideSep %in% as.character(sample_freq_sub$Var1)) 
        ...
        #saveRDS(cont_tab, paste("ALL_hGEPs_nrSpots_per_Density_usagethreshold", thresh, "_nrSpotThresh", NRSPOT, ".rds",sep=""))


        pdf(paste("ALL_hGEPs_chisq_plot_usagethreshold", thresh, "_nrSpotThresh", NRSPOT, ".pdf",sep=""), width = 15, height = 5)
        for(s in 1:length(cont_tab)){
            ...
            if(length(which(rowSums(df1) == 0)) > 0){
                chsq_DFlist[[1]] <- df1[which(rownames(df1) %in% intersect(rownames(df1), c("Tumor_D1","Tumor_D2","Tumor_D3","Tumor_D4"))),]
...
write.table(TAB_all, "hGEP_chisq_all_samples_per_program.txt", sep="\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
